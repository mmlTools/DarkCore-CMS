...
<?php /*   <div id='content'>
            ...
            foreach($board->categorys as $category){ ?>
                <div id='forum-category-left'>
                    ...
                    foreach ($forums->forums as $forum){
                        ...
                        if (!is_null($forums->forums[$j]['description'])) { ?>
                            <div class="forum-line">
                                <a href="forum=<?php echo $forums->forums[$j]['id']; ?>/<?php urlencode($forums->forums[$j]['title']); ?>">
                                ...
                                <div class="f-info-latest">
                                        <a href="topic=<?php echo $lasttopic['id']; ?>/<?php echo urlencode($lasttopic['title']);?>">
                                            <div style="color:#A1E8B9;padding-top:5px;font-size:12px">
                                                <?php
                                                if (strlen($lasttopic['title'])>24) 
                                                    echo substr($lasttopic['title'], 0, 24).'...';
                                                else 
                                                    echo $lasttopic['title'];
                                                ?>
                                            </div>
                                        </a>
                                        <a href="player?id=<?php echo $lasttopic['autor']; ?>">
                                            <div style="color:#<?php echo namecolor(get_rank_byid($lasttopic['autor'])); ?>;font-size:12px">
                                                <?php echo ucfirst(strtolower(get_username_byid($lasttopic['autor']))); ?>
                                            </div>
                                        </a>
    ...
    <?php } else { ?>
                            <div class='area-forum-line'>
                                <a href="forum=<?php echo $forums->forums[$j]['id'] ."/". urlencode($forums->forums[$j]['title']); ?>">
                                    <div class='forum-icon-place'>
                                        <img src="<?php echo $forums->forums[$j]['icon'];?>" class="forum-icon"/>
                                    </div>
                                    <div class='area-f-title-desc'>
                                        <div class='area-f-title'><?php echo $forums->forums[$j]['title']; ?></div>
                                        <div class='area-f-desc'>
                                            Topics: <?php echo $topic->total_topics; ?>
                                            Posts: <?php echo $topic->total_posts; ?>
                                        </div>
                                    </div>
                                </a>
                            </div>
    ...
    <?php $i++; }?>
        ...
        <div id='forum-right'>
            ...
            <?php foreach($stats->latest_topics as $topics) { ?>
                    <div class='right-info-latest'>
                        <a href="topic=<?php echo $stats->latest_topics[$i]['id'] . "/". urlencode($stats->latest_topics[$i]['title']);?>">
                        <div style="color:#A1E8B9;padding-top:5px;font-size:16px">
                            <?php echo $stats->latest_topics[$i]['title']; ?>
                        </div>
                    </a>
                    <a href="player?id=<?php echo $stats->latest_topics[$i]['autor']; ?>">
                        <div style="color:#<?php echo namecolor(get_rank_byid($stats->latest_topics[$i]['autor'])); ?>;font-size:14px">
                            <?php echo ucfirst(strtolower(get_username_byid($stats->latest_topics[$i]['autor']))); ?>
                        </div>
                    </a>
                    <div style='color:#717B7A;font-size:12px'>
                        <?php echo $stats->latest_topics[$i]['date']; ?>
                    </div>
                </div>
            <?php $i++; } $i=1;?>
        ...
        <?php foreach($stats->latest_posts as $posts) { 
            ...
            ?>
            <div class='right-info-latest'>
                <a href="topic=<?php echo $topicbyid['id'] ."/". urlencode($topicbyid['title']);?>">
                    <div style='color:#A1E8B9;padding-top:5px;font-size:16px'>
                        <?php echo $stats->latest_posts[$i]['body']; ?>
                    </div>
                </a>
                <a href="player?id=<?php echo $stats->latest_posts[$i]['autor']; ?>">
                    <div style="color:#<?php echo namecolor(get_rank_byid($stats->latest_posts[$i]['autor'])); ?>;font-size:14px">
                        <?php echo ucfirst(strtolower(get_username_byid($stats->latest_posts[$i]['autor']))); ?>
                    </div>
                </a>
                <div style="color:#717B7A;font-size:12px">
                    <?php echo $stats->latest_posts[$i]['date']; ?>
                </div>
            </div>
        <?php $i++; } ?>
